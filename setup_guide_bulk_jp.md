...
> **💡 解説**：edgeR は Bioconductor のパッケージで、DEG解析の中核を担います。依存パッケージの limma も自動的にインストールされます。rmarkdown / knitr は解析結果をHTMLレポートとして出力するために使用します。
...
> **💡 解説**：本実習で使用するデータは、健常者（HC）および全身性エリテマトーデス（SLE）患者の末梢血から28種類の免疫細胞をFACSで分離し、RNA-seqを行ったものです。DEG解析ではTh1細胞のHC vs SLEの比較を行います。
...
packages <- c("ggplot2", "dplyr", "edgeR", "ggrepel", "gplots", "limma", "variancePartition", "rmarkdown", "knitr")
...
```r
# CRANパッケージの場合
install.packages(c("ggplot2", "dplyr", "ggrepel", "gplots", "rmarkdown", "knitr"))
...
解析スクリプト（.R）の実行結果を、コード・出力・プロットをまとめたHTMLレポートとして保存できます。

### 方法：RStudioでRスクリプトをレンダリング
...
> **💡 ヒント**：`.R` ファイルを直接レンダリングすると、`#'` で始まるコメントがマークダウンとして、それ以外がコードチャンクとして処理されます。
