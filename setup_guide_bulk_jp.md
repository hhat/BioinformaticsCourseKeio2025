...
> 既にRStudioをお持ちの方も、本ガイドの手順に沿ってconda環境から起動してください。
> 普段の起動方法（Applicationsから開く等）では、必要なパッケージが使えない場合があります。
...
> **💡 既にcondaをインストール済みの方**：この手順はスキップしてセクション3に進んでください。
...
```bash
# bulkworkshop という名前の環境を作成（R と RStudio を同時にインストール）
conda create -n bulkworkshop -c conda-forge r-base=4.3.2 rstudio -y
```
...
```bash
conda create -n bulkworkshop -c conda-forge r-base=4.3.2 -y
```

> **⏱ 注意：初回のインストールには10〜20分程度かかることがあります。**

> **⚠️ バージョン指定でエラーが出る場合**：`=4.3.2` を削除して再実行してください。
> ```bash
> # macOS
> conda create -n bulkworkshop -c conda-forge r-base rstudio -y
> # Windows
...
> ```bash
> conda install -c conda-forge r-ggplot2 r-dplyr r-ggrepel r-ggsci r-gplots -y
> ```

### Step D. Bioconductorパッケージのインストール

```bash
conda install -c bioconda -c conda-forge bioconductor-edger -y
```
...
### パターン1：conda環境にRStudioがある場合（macOS でStep Aを実行した方）
...
> **⚠️ 重要**：必ずターミナルから上記のコマンドで起動してください。
> Applications フォルダや Dock から RStudio を開くと、conda環境の R が使用されません。

### パターン2：RStudioをアプリケーションとして別途インストールしている場合

既に RStudio をお持ちの方（`open -a RStudio` や Applications から起動していた方）は、conda環境の R を使うように設定する必要があります。

#### macOS の場合

```bash
conda activate bulkworkshop
# conda環境のRを使うようにRStudioに指示
export RSTUDIO_WHICH_R=$(which R)
open -a RStudio
```

> **💡 確認方法**：RStudio が起動したら、コンソール（左下パネル）で以下を実行してください：
> ```r
> R.home()
> ```
> 出力に `miniforge3` や `bulkworkshop` を含むパスが表示されていれば正しく設定されています。
> もし `/Library/Frameworks/R.framework/...` のようなパスが表示される場合は、conda環境の R が使われていません。
> その場合はターミナルから `rstudio &` で起動し直してください。
...
##### RStudioの起動（Miniforge Prompt から）
...
├── img/
│   ├── database_overview.png          (データベース概要図)
│   ├── facs_gating.png                (細胞ソーティング図)
│   └── pca_concept.png                (PCA概念図)
└── data/
    └── bulk/
        ├── sample_meta.txt            (サンプルメタデータ)
        ├── celltype_list              (細胞タイプリスト)
        ├── IFNgenes100.txt            (IFN関連遺伝子リスト)
        └── count/
            ├── Th1_count.txt          (Th1カウントデータ)
            ├── Th2_count.txt          (Th2カウントデータ)
            ├── ...                    (各細胞タイプのカウントデータ)
...
packages <- c("ggplot2", "dplyr", "edgeR", "ggrepel", "ggsci", "gplots", "limma")
...
**対処法**：必ずターミナル / Miniforge Prompt から `conda activate bulkworkshop` を実行した後にRStudioを起動してください（セクション4参照）。
...
```r
# CRANパッケージの場合
install.packages(c("ggplot2", "dplyr", "ggrepel", "ggsci", "gplots"))
